...
namespace EspadaLendaria {
    ...
    namespace Menu {
        ...
        Menu::Menu(const IDs::IDs ID, const sf::Vector2f tamBotao, const std::string nome, const unsigned int tamFonte) :
            ...
            titulo(pGrafico->carregarFonte("fonte/Ancient Medium.ttf"), nome, tamFonte), //créditos da fonte a artRAVE Editions do site: https://www.dafont.com/pt/search.php?q=ancient
            observadorMenu(new Observador::ObservadorMenu(this)), mouseSelecionado(false)
        {
            if (observadorMenu == nullptr) {
                std::cout << "ERROR::EspadaLendaria::Menu::MenuPrincipal::nao foi possivel criar um Observador Menu Principal" << std::endl;
                exit(1);
            }
        }
        ...
        void Menu::addBotao(const std::string info, const sf::Vector2f pos, const IDs::IDs ID, const sf::Color corSelecionado) {
            ...
            if (botao == nullptr) {
                throw("ERROR::EspadaLendaria::Menu::nao foi possivel criar um botao");
            }
